...
public class ClassUtils {
    ...
    private static SharedPreferences getMultiDexPreferences(Context context) {
        return context.getSharedPreferences(PREFS_FILE, Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB ? Context.MODE_PRIVATE : Context.MODE_MULTI_PROCESS);
    }
            ...
            DefaultPoolExecutor.getInstance().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        Logger.e("Scan map file in dex files made error." + ignore.toString());
                    } finally {
                        if (null != dexfile) {
                            try {
                                dexfile.close();
...
//        通过是否存在sp中的multidex.version是不准确的，因为从低版本升级上来的用户，是包含这个sp配置的
        ...
        if (Component.debuggable()) { // Search instant run support only debuggable
