...
public class ZipUtils {
    ...
    //第二种实现
    public static void zip(String src, String dest, FilenameFilter fileFilter) throws IOException {
        ...
        try {
            ...
            if (fileOrDirectory.isFile()) {
                zipFileOrDirectory(out, fileOrDirectory, "", fileFilter);
            } else {
                // 返回一个文件或空阵列。
                File[] entries = fileOrDirectory.listFiles();
                for (int i = 0; i < entries.length; i++) {
                    // 递归压缩，更新curPaths
                    zipFileOrDirectory(out, entries[i], "", fileFilter);
                }
            }
    ...
    private static void zipFileOrDirectory(ZipOutputStream out,
        ...
        try {
            ...
            if (!fileOrDirectory.isDirectory()) {
                ...
                int bytes_read;
                String name = curPath + fileOrDirectory.getName();
                ...
                for (int i = 0; i < entries.length; i++) {
                    ...
                    zipFileOrDirectory(out, entries[i], curPath
                            + fileOrDirectory.getName() + "/", filter);
                }
            }
        } catch (IOException ex) {
            ex.printStackTrace();
